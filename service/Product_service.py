...
class ProductService:
    def __init__(self, table=None):
        if table:
            ...
            self.table = get_table(ProductModel)

    def find(self, params, id=None):
        if id is None:
            logging.info('Finding all records of Product...')
            found = self.table.find_all(
                20,
                self.table.get_conditions(params, False)
            )
        else:
            logging.info(f'Finding "{id}" in Product ...')
    ...
    def insert(self, json):
        logging.info('New record write in Product')
    ...
    def update(self, json):
        logging.info('Changing record of Product ...')
    ...
    def delete(self, id):
        logging.info('Removing record of Product ...')
